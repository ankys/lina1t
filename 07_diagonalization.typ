...
\$\$braket(bold(u), bold(v)} = 0\$\$ が成り立つことをいう。
定義上は\$bold(u) = bold(0)\$または\$bold(v) = bold(0)\$が成り立っていたら\$bold(u)\$と\$bold(v)\$は必ず直交している。
また、ベクトル\$bold(v)\$が_単位ベクトル_とは
\$\$braket(bold(v), bold(v)} = 1\$\$ が成り立つことをいう。

$V$の基底\$bold(v)\_1, dots, bold(v)\_N\$が各$i eq.not j$,
$i, j = 1, dots, N$に対して\$bold(v)\_i\$と\$bold(v)\_j\$が直交することを満たす時、_直交基底_であるという。
直交基底がさらに各$i = 1, dots, N$に対して\$bold(v)\_i\$が単位ベクトルである時、_正規直交基底_であるという。

本節の目標は$V$の基底が与えられた時に直交基底あるいは正規直交基底に取り替えることである。
そのことを表現するために$V$の$N = 1, dots$個の零ベクトルでないベクトル\$bold(v)\_1, dots, bold(v)\_N\$が各$i eq.not j$,
$i, j = 1, dots, N$に対して\$bold(v)\_i\$と\$bold(v)\_j\$が直交することを満たす時、$V$の_直交系_であるということにする。
...
#block[
_Proof.] スカラー$c_1, dots, c_N in K$に対して
\$\$c\_1bold(v)\_1+dots+c\_Nbold(v)\_N = bold(0)\$\$
が成り立ったとする時、各$i = 1, dots, N$に対して、\$bold(v)\_i\$との内積を取って
\$\$braket(bold(v)\_i, c\_1bold(v)\_1+dots+c\_Nbold(v)\_N} = c\_ibraket(bold(v)\_i, bold(v)\_i} = 0.\$\$
...
そのベクトル\$bold(v) = c\_1bold(v)\_1+dots+c\_Nbold(v)\_N in W\$と$i = 1, dots, N$に対して、\$bold(v)\_i\$との内積を取ると\$braket(bold(v)\_i, bold(v)} = c\_ibraket(bold(v)\_i, bold(v)\_i}\$でありこのスカラー値$c_i$のことを\$bold(v)\$の\$bold(v)\_i\$_成分_と呼ぶ。
...
&bold(u)\_1 = bold(v)\_1,
quad
bold(u)\_2 = bold(v)\_2-\\frac{braket(bold(u)\_1, bold(v)\_2}}{braket(bold(u)\_1, bold(u)\_1}}bold(u)\_1,
quad
bold(u)\_3 = bold(v)\_3-\\frac{braket(bold(u)\_1, bold(v)\_3}}{braket(bold(u)\_1, bold(u)\_1}}bold(u)\_1-\\frac{braket(bold(u)\_2, bold(v)\_3}}{braket(bold(u)\_2, bold(u)\_2}}bold(u)\_2, \\\\
&quad dots,
quad
bold(u)\_N = bold(v)\_N-\\frac{braket(bold(u)\_1, bold(v)\_N}}{braket(bold(u)\_1, bold(u)\_1}}bold(u)\_1-dots-\\frac{braket(bold(u)\_{N-1}, bold(v)\_N}}{braket(bold(u)\_{N-1}, bold(u)\_{N-1}}}bold(u)\_{N-1}.
\\end{aligned}\$\$
このとき\$bold(u)\_1, dots, bold(u)\_N\$は$V$の直交系であり、\$braket(bold(u)\_1, dots, bold(u)\_N} = braket(bold(v)\_1, dots, bold(v)\_N}\$が成り立つ。

]
#block[
_Proof.] $N$についての数学的帰納法による。
$N = 1$の時、\$bold(u)\_1 = bold(v)\_1 \\ne bold(0)\$より成立する。
$N$個で成立した時、$N+1$個目のベクトルを
\$\$bold(u)\_{N+1} = bold(v)\_{N+1}-\\frac{braket(bold(u)\_1, bold(v)\_{N+1}}}{braket(bold(u)\_1, bold(u)\_1}}bold(u)\_1-dots-\\frac{braket(bold(u)\_N, bold(v)\_{N+1}}}{braket(bold(u)\_N, bold(u)\_N}}bold(u)\_N\$\$
で定めると、\$bold(u)\_1, dots, bold(u)\_N\$は$V$の直交系より、各$i = 1, dots, N$に対して\$bold(u)\_i\$との内積を取って、
\$\$braket(bold(u)\_i, bold(u)\_{N+1}} = braket(bold(u)\_i, bold(v)\_{N+1}}-\\frac{braket(bold(u)\_i, bold(v)\_{N+1}}}{braket(bold(u)\_i, bold(u)\_i}}braket(bold(u)\_i, bold(u)\_i} = 0.\$\$
また、\$bold(u)\_{N+1} = bold(0)\$とすると \$\$bold(v)\_{N+1}
= \\frac{braket(bold(u)\_1, bold(v)\_{N+1}}}{braket(bold(u)\_1, bold(u)\_1}}bold(u)\_1+dots+\\frac{braket(bold(u)\_N, bold(v)\_{N+1}}}{braket(bold(u)\_N, bold(u)\_N}}bold(u)\_N
...
\$\$\\mqty(bold(v)\_1 & dots & bold(v)\_N)
= \\mqty(bold(u)\_1 & dots & bold(u)\_N)\\mqty(1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & 1)\$\$
と表示できる。
...
\$\$K\_{\\ge 0} = \\lrset{ overline(c\_1}c\_1+dots+overline(c\_n}c\_n \\mid n = 1, 2, 3, dots, c\_1, dots, c\_n in K }\$\$
とする。 ここで正値$c in K_(gt.eq 0)$に対して、 $ overline(x) x = c $
を満たす正値$x in K_(gt.eq 0)$が一意に存在する時、$x$を$c$の_正の平方根_といい$sqrt(c)$と表す。
さらに任意の$c in K_(gt.eq 0)$に対して正の平方根$sqrt(c)$が存在する時、$K$は_正規化可能_と呼ぶ。

]
#block[
$0$は$0$の正の平方根であり、$1$は$1$の正の平方根である。
$overline(x) x = c$と書いたが、正値$x in K_(gt.eq 0)$の共役は
$ overline(x) = overline(overline(c_1) c_1+dots+overline(c_n) c_n) = c_1 overline(c_1)+dots+c_n overline(c_n) = x $
...
しかしながら、非負の有理数$2 = overline(1) dot 1+overline(1) dot 1$に対して、$sqrt(2)$は無理数なので、$bb(Q)$は正規化可能でない。
...
#block[
正規化可能な体_K_上の数ベクトル\$bold(v)\$に対して、\$braket(bold(v), bold(v)}\$は$K_(gt.eq 0)$の元である。
さらにその正の平方根を\$\\norm{bold(v)} = \\sqrt{braket(bold(v), bold(v)}} in K\_{\\ge 0}\$とおく。
ここで、\$bold(v) \\ne bold(0)\$に対して
\$\$\\tilde{bold(v)} = \\norm{bold(v)}^{-1}bold(v)\$\$
とおくとこれは単位ベクトルになっている。

]
このときの\$\\norm{bold(v)}\$の_ノルム_または_大きさ_あるいは_長さ_という。
...
\$\$overline(A}^T = \\mqty(overline(a\_{1 1}} & dots & overline(a\_{M 1}} \\\\ \\vdots & \\ddots & \\vdots \\\\ overline(a\_{1 N}} & dots & overline(a\_{M N}})\$\$
として定義される。
...
\$\$A = \\bar{P}\\mqty(1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & 1)\\mqty(c\_1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & c\_N)\\mqty(1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & 1)^{-1}\\bar{P}^{-1}\$\$
となる。 よって、右上三角行列の積は右上三角行列であることに注意して、
\$\$A = \\bar{P}\\mqty(c\_1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & c\_N)\\bar{P}^{-1}\$\$
とできる。 さらに正規化して正規直交行列 \$\$\\tilde{P}
= \\mqty(\\norm{bold(u)\_1}^{-1}bold(u)\_1 & dots & \\norm{bold(u)\_N}^{-1}bold(u)\_N)
= \\bar{P}\\mqty(\\dmat{\\norm{bold(u)\_1}^{-1}, \\ddots, \\norm{bold(u)\_N}^{-1}})\$\$
を定めると、 \$\$A
= \\tilde{P}\\mqty(\\dmat{\\norm{bold(u)\_1}, \\ddots, \\norm{bold(u)\_N}})\\mqty(c\_1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & c\_N)\\mqty(\\dmat{\\norm{bold(u)\_1}^{-1}, \\ddots, \\norm{bold(u)\_N}^{-1}})\\tilde{P}^{-1}
= \\tilde{P}\\mqty(c\_1 & dots & \* \\\\ & \\ddots & \\vdots \\\\ & & c\_N)\\tilde{P}^{-1}\$\$
とできる。~◻
