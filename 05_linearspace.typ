...
#block[
$N = 1 , 2 , 3 , dots.h.c$とする。
$V$として直積$K^N$として設定するつまり$N$個の$K$の元$a_1 , dots.h.c , a_N$を並べたもの\$\\vb\*{v} = (a\_1, \\cdots, a\_N)\$の集合として、加法とスカラー乗法を
$ (a_1 , dots.h.c , a_N) + (b_1 , dots.h.c , b_N) = (a_1 + b_1 , dots.h.c , a_N + b_N) , quad c (a_1 , dots.h.c , a_N) = (c a_1 , dots.h.c , c a_N) $
...
#block[
同様にして$M , N = 1 , 2 , 3 , dots.h.c$とする。
$V$として$M times N$型の$K$上の行列の集合\$\\M\_{M\\times N}(K) = K^{M\\times N}\$とすると$V$は$K$上の線形空間である。

]
#block[
$V$を$K$の元を並べた数列$(a_n)_(n in bb(N))$全体$V = K^(bb(N))$として加法とスカラー乗法を
$ (a + b)_n = a_n + b_n , quad (c a)_n = c a_n $
...
#block[
実数は和と有理数倍が定義できるので、$bb(R)$は有理数体$bb(Q)$上の線形空間である。

]
#block[
正の実数$bb(R)_(+)$の「和」を$x plus.square y = x y$で「実数$c$倍」を$c dot.square x = x^c$で定義すると、
$bb(R)_(+)$は実数体$bb(R)$上の線形空間である。
...
線形空間$V$に対して、$V$自身と零ベクトルのみからなる集合\$O = O\_V = \\{ \\vb\*{0}\_V \\}\$はいずれも$V$の線形部分空間であり、自明な線形部分空間と呼ばれる。

#block[
$N , M = 1 , 2 , 3 , dots.h.c$として$A$を$M times N$型の$K$上の行列とする。
この時、斉次連立一次方程式 \$\$A\\vb\*{x} = \\vb\*{0}\$\$
の解\$\\vb\*{x} \\in K^N\$全体からなる集合は$K^N$の線形部分空間である。

]
#block[
$N = 1 , 2 , 3 , dots.h.c$次の対角行列、右上三角行列、左下三角行列、対称行列の集合はいずれも\$\\M\_N(K)\$の線形部分空間である。

]
#block[
$K$の元を係数とする$N = 0 , 1 , 2 , 3 , dots.h.c$次以下の多項式
$ f (x) = a_0 + a_1 x + a_2 x^2 + a_3 x^3 + dots.h.c + a_N x^N , quad a_0 , a_1 , a_2 , a_3 , dots.h.c , a_N in K $
...
は有理数体$bb(Q)$上の線形空間としての$bb(R)$の線形部分空間である。
...
ここでスカラー$c_1 , dots.h.c , c_N in K$を使って
\$\$c\_1 \\vb\*{v}\_1+\\cdots+c\_N \\vb\*{v}\_N\$\$
と表される$V$の元（ベクトル）を\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$の#emph[線形結合];という。
また、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$の線形結合全体からなる集合
\$\$\\lrangle{ \\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N } = \\lrset{ c\_1 \\vb\*{v}\_1+\\cdots+c\_N \\vb\*{v}\_N \\mid c\_1, \\cdots, c\_N \\in K }\$\$
...
- 交空間が$V sect W = O_U$を満たす時、和空間$V + W$は$V xor W$と書かれ、$V$と$W$の#emph[直和空間];と呼ばれる。
...
ここで$V$の線形部分空間$U$であって$W xor U = V$、つまり$W sect U = O_V$かつ$W + U = V$を満たすものを$W$の#emph[線形補空間];という。
...
#block[
$V$を線形空間として、いくつかのベクトル\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$
($N = 0 , 1 , 2 , 3 , dots.h.c$)が存在して
...
また、後でわかることとして
$ dim K^N = N , quad dim K^(M times N) = M N , quad dim K^(bb(N)) = oo , quad dim K [x] = oo , quad dim K^X = \# X , quad dim K^N [x] = N + 1 $
である。
ただし$\# X$は集合$X$の元の個数（有限集合でない場合は$\# X = oo$）である。
$N$個のベクトル\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N \\in V\$が$V$を線形生成する時、$N gt.eq dim V$であることに注意する。
...
ここでスカラー$c_1 , dots.h.c , c_N in K$であって
\$\$c\_1 \\vb\*{v}\_1+\\cdots+c\_N \\vb\*{v}\_N = \\vb\*{0}\$\$
となるのは$c_1 = dots.h.c = c_N = 0$の場合のみとする時、
...
#block[
$N = 0 , 1 , 2 , 3 , dots.h.c$として$1 , x , x^2 , x^3 , dots.h.c , x^N in K [x]$は線形独立である。
...
#block[
#emph[Proof.]
$V$の次元を$N gt.eq 1$、基底を\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N \\in V\$とする。
もし\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$が線形独立でないつまり線形従属している時は、$c_1 = dots.h.c = c_N = 0$でないスカラー$c_1 , dots.h.c , c_N in K$であって
\$\$c\_1 \\vb\*{v}\_1+\\cdots+c\_N \\vb\*{v}\_N = \\vb\*{0}\$\$
となるものが存在する。
ここで$c_i eq.not 0$となる$i = 1 , dots.h.c , N$が存在するが、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$を並べ替えて、$i = N$の場合のみ考えればよい。
この時、
\$\$\\vb\*{v}\_N = -c\_N^{-1}(c\_1 \\vb\*{v}\_1+\\cdots+c\_{N-1} \\vb\*{v}\_{N-1})\$\$
となるため、\$\\vb\*{v}\_N\$は\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_{N-1}\$の線形結合として書け、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_{N-1}\$は$V$を線形生成する。
実際、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$が$V$を線形生成するため任意の$w in V$は\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$の線形結合として書けるので、$d_1 , dots.h.c , d_N in K$を使って
\$\$w = d\_1\\vb\*{v}\_1+\\cdots+d\_N\\vb\*{v}\_N
= d\_1\\vb\*{v}\_1+\\cdots+d\_{N-1}\\vb\*{v}\_{N-1}-d\_N c\_N^{-1}(c\_1 \\vb\*{v}\_1+\\cdots+c\_{N-1} \\vb\*{v}\_{N-1})\$\$
...
#block[
$V$を$K$上の線形空間として、$N = 0 , 1 , 2 , 3 , dots.h.c$個のベクトル\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N \\in V\$が線形独立ならば$N lt.eq dim V$である。
...
#block[
#emph[Proof.]
$M = dim V$として$M , N = 1 , 2 , 3 , dots.h.c$の場合を考えればよい。
つまり、$V$は次元が$M$の有限次元である。
そこで$V$の基底\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$を取ってくる。
各$j = 1 , dots.h.c , N$に対して\$\\vb\*{v}\_i \\in V\$は\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$の線形結合として表されるので、
\$\$\\vb\*{v}\_j = c\_{j 1}\\vb\*{w}\_1+\\cdots+c\_{j M}\\vb\*{w}\_M\$\$
となる$c_(j 1) , dots.h.c , c_(j M) in K$が存在する。
ここで\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$は線形独立なので、$d_1 , dots.h.c , d_N in K$が\$d\_1\\vb\*{v}\_1+\\cdots+d\_N\\vb\*{v}\_N = \\vb\*{0}\$を満たすならば$d_1 = dots.h.c = d_N = 0$である。
\$\$\\begin{aligned}
d\_1\\vb\*{v}\_1+\\cdots+d\_N\\vb\*{v}\_N
&= d\_1(c\_{1 1}\\vb\*{w}\_1+\\cdots+c\_{1 M}\\vb\*{w}\_M)+\\cdots+d\_N(c\_{N 1}\\vb\*{w}\_1+\\cdots+c\_{N M}\\vb\*{w}\_M) \\\\
&= (d\_1 c\_{1 1}+ d\_N c\_{N 1})\\vb\*{w}\_1+\\cdots+(d\_1 c\_{1 M}+ d\_N c\_{N M})\\vb\*{w}\_M
\\end{aligned}\$\$
なので、\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$が線形独立であることから
$ d_1 c_(1 1) + d_N c_(N 1) = dots.h.c = d_1 c_(1 M) + d_N c_(N M) = 0 $
で、これを変形して斉次連立一次方程式
$ mat(delim: "(", c_(1 1), dots.h.c, c_(N 1); dots.v, dots.down, dots.v; c_(1 M), dots.h.c, c_(N M); #none) vec(d_1, dots.v, d_N, ) = vec(0, dots.v, 0, ) $
を得る。
ここで定理@t:homlinsysより係数行列の階数を$R$とすると、この方程式の解が$d_1 = dots.h.c = d_N = 0$に限られる条件は$R = N$であることである。
...
#block[
#emph[Proof.]
スカラー$c_1 , dots.h.c , c_N , d in K$を使って\$c\_1\\vb\*{v}\_1+\\cdots+c\_N\\vb\*{v}\_N+d\\vb\*{w} = \\vb\*{0}\_V\$となったとする。
ここで$d eq.not 0$とすると
\$\$\\vb\*{w} = -d^{-1}(c\_1\\vb\*{v}\_1+\\cdots+c\_N\\vb\*{v}\_N)\$\$
となり仮定に反する。
よって$d = 0$であり、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$が線形独立なので$c_1 = dots.h.c = c_N = 0$である。
...
定理の後半部分の内容を先に示しておく。
つまり$V sect W = O_U$の時、$V$の基底\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$と$W$の基底\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$を取ってくると\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N, \\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$が線形独立を示せばよい。
スカラー$c_1 , dots.h.c , c_N , d_1 , dots.h.c , d_M$について
\$\$c\_1\\vb\*{v}\_1+\\cdots+c\_N\\vb\*{v}\_N+d\_1\\vb\*{w}\_1+\\cdots+d\_M\\vb\*{w}\_M = \\vb\*{0}\_U\$\$
の時、
\$\$c\_1\\vb\*{v}\_1+\\cdots+c\_N\\vb\*{v}\_N = -d\_1\\vb\*{w}\_1-\\cdots-d\_M\\vb\*{w}\_M\$\$
であり、仮定からこれは$V sect W = O_U$の元より\$\\vb\*{0}\_U\$に等しく、
...
一般の場合を示す。
$V sect W$は有限次元線形空間$W$の線形部分空間なので、命題@t:complementより、線形補空間が存在しそれぞれ$W'$とおく。
ここで$V + W = V xor W'$を示す。
...
#block[
$bb(R)$を$bb(R)$上の線形空間と考えて関数$f : bb(R) arrow.r bb(R)$を考える。
...
- $F$が単射である、つまり任意の\$\\vb\*{v}, \\vb{w} \\in V\$に対して\$F\\vb\*{v} = F\\vb\*{w}\$ならば\$\\vb\*{v} = \\vb\*{w}\$が成り立つ、ための必要十分条件は\$\\Ker F = O\_V\$が成り立つことである。
...
#block[
#emph[Proof.] $c , d in K$と\$\\vb\*{v}, \\vb\*{w} \\in W\$を取る。
示すべきことは
...
$V , W$に対してその間の線形同型写像が存在する時$V$と$W$は#emph[線形同型];であるといい、$V tilde.equiv W$と表す。
...
#block[
#emph[Proof.]
\$\\Img F\$が無限次元の時は任意の$N = 0 , 1 , 2 , 3 , dots.h.c$に対して線形独立なベクトル\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_N \\in \\Img F\$が存在する。
ここで各\$\\vb\*{w}\_i\$に対して\$F\\vb\*{u}\_i = \\vb\*{w}\_i\$となる\$\\vb\*{u}\_i \\in V\$が存在するので取ってくる。
この時\$\\vb\*{u}\_1, \\cdots, \\vb\*{u}\_N\$は線形独立を示す。
スカラー$c_1 , dots.h.c , c_N in K$に対して
\$\$c\_1\\vb\*{u}\_1+\\cdots+c\_N\\vb\*{u}\_N = \\vb\*{0}\_V\$\$
となったとすると$F$で写して
\$\$F(c\_1\\vb\*{u}\_1+\\cdots+c\_N\\vb\*{u}\_N)
= c\_1 F\\vb\*{u}\_1+\\cdots+c\_N F\\vb\*{u}\_N
= c\_1\\vb\*{w}\_1+\\cdots+c\_N\\vb\*{w}\_N
= F(\\vb\*{0}\_V)
= \\vb\*{0}\_W.\$\$
よって、\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_N\$が線形独立なので、$c_1 = dots.h.c = c_N = 0$であり、\$\\vb\*{u}\_1, \\cdots, \\vb\*{u}\_N\$も線形独立である。
...
あとは\$U\\cap \\Ker F = O\_V\$と\$U+\\Ker F = V\$を示せばよい。
まず\$\\vb\*{v} \\in U\\cap \\Ker F\$とすると、
\$\$\\vb\*{v} = c\_1\\vb\*{u}\_1+\\cdots+c\_N\\vb\*{u}\_N\$\$
とでき$F$で移すと \$\$F(c\_1\\vb\*{u}\_1+\\cdots+c\_N\\vb\*{u}\_N)
= c\_1 F\\vb\*{u}\_1+\\cdots+c\_N F\\vb\*{u}\_N
= c\_1\\vb\*{w}\_1+\\cdots+c\_N\\vb\*{w}\_N
= F(\\vb\*{v})
= \\vb\*{0}\_W\$\$
よって$c_1 = dots.h.c = c_N = 0$で\$\\vb\*{v} = \\vb\*{0}\_V\$である。
次に\$\\vb\*{v} \\in V\$に対して、\$F\\vb\*{v} \\in \\Img F\$より\$F\\vb\*{v} = c\_1\\vb\*{w}\_1+\\cdots+c\_N\\vb\*{w}\_N\$とでき、
\$\$F(\\vb\*{v}-c\_1\\vb\*{u}\_1-\\cdots-c\_N\\vb\*{u}\_N)
= F\\vb\*{v}-c\_1 F\\vb\*{u}\_1-\\cdots-c\_N F\\vb\*{u}\_N
= F\\vb\*{v}-c\_1\\vb\*{w}\_1-\\cdots-c\_N\\vb\*{w}\_N
= \\vb\*{0}\_W.\$\$
よって、\$\\vb\*{v}-c\_1\\vb\*{u}\_1-\\cdots-c\_N\\vb\*{u}\_N \\in \\Ker F\$である。
...
#block[
$N = 0 , 1 , 2 , 3 , dots.h.c$とする。
$K$上の$N$次元数ベクトル空間$K^N$は有限次元で$dim K^N = N$であり、$N$個のベクトル
\$\$\\vb\*{e}\_1 = (1, 0, \\cdots, 0),
\\quad \\cdots,
\\quad \\vb\*{e}\_N = (0, \\cdots, 0, 1)\$\$ が$K^N$の基底である。

]
#block[
#emph[Proof.]
\$\\vb\*{e}\_1, \\cdots, \\vb\*{e}\_N\$が線形独立かつ$K^N$を線形生成ことを示せばよい。
線形結合はスカラー$c_1 , dots.h.c , c_N in K$に対して
\$\$c\_1\\vb\*{e}\_1+\\cdots+c\_N\\vb\*{e}\_N = (c\_1, \\cdots, c\_N)\$\$
なので、
これが零ベクトル$(0 , dots.h.c , 0)$となるのは$c_1 = dots.h.c = c_N = 0$であり、
...
\$\$F(c\_1, \\cdots, c\_N) = c\_1\\vb\*{v}\_1+\\cdots+c\_N\\vb\*{v}\_N\$\$
で定義する。
このとき、\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$が$V$を線形生成することから$F$は全射で、
線形独立であることから\$\\Ker F = O\_{K^N}\$つまり$F$は単射である。
以上より$F$は線形同型写像であるので、$V tilde.equiv K^N$である。~◻

]
$M , N = 1 , 2 , 3 , dots.h.c$として$A$を$K$上の$M times N$型の行列とする。
ここで$K^N$から$K^M$への写像 \$\$F\_A(\\vb\*{v}) = A\\vb\*{v}\$\$
...
$F$を$V$から$W$への線形写像として$V$の基底\$\\vb\*{v}\_1, \\cdots, \\vb\*{v}\_N\$と$W$の基底\$\\vb\*{w}\_1, \\cdots, \\vb\*{w}\_M\$について、各$j = 1 , dots.h.c , N$に対して
\$\$F(\\vb\*{v}\_j) = a\_{j 1}\\vb\*{w}\_1+\\cdots+a\_{j M}\\vb\*{w}\_M\$\$
となる$a_(j 1) , dots.h.c , a_(j M) in K$が一意に存在して定義される$K$上の$M times N$型の行列
$ mat(delim: "(", a_(1 1), dots.h.c, a_(N 1); dots.v, dots.down, dots.v; a_(1 M), dots.h.c, a_(N M); #none) $
...
$K$上の線形空間$V$の元\$\\vb\*{v}\$を\$\\vb\*{v}\$に移す線形変換を特に#emph[恒等変換];といい、\$\\Id\_V\$あるいは単に\$\\Id\$と表す。
