...
#block[
複素数の中でも有理数$bb(Q)$上の$1$次以上の多項式$f (x) in bb(Q) [x]$の零点となる数は_代数的数_と呼ばれる。
代数的数全体の集合
\$\$\\bar{\\mathbb{Q}} = \\lrset{ a in \\mathbb{C} mid(|) a^n+c\_{n-1}a^{n-1}+dots+c\_0 = 0, n = 1, 2, 3, dots, c\_0, dots, c\_{n-1} in \\mathbb{Q} }\$\$
...
このとき、\$P = \\mqty(bold(v)\_1 & dots & bold(v)\_N)\$を使って、
