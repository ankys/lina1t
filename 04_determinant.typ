...
つまり$2$次正方行列$A = mat(delim: "(", a, b; c, d)$の行列式の値は
$ det A = det mat(delim: "(", a, b; c, d) = mat(delim: "||", a, b; c, d) = a d - b c $
である。
...
= c\\det\\begin{pmatrix}\\vb\*{a} & \\vb\*{b}\\end{pmatrix}+c\'\\det\\begin{pmatrix}\\vb\*{a} & \\vb\*{b}\'\\end{pmatrix}\$\$
が成り立つことと、 二つの$2$次正方行列$A$, $B$に対して積の行列式の等式
$ det (A B) = det A det B $ が成り立っていることがわかる。
...
つまり、
$ s = mat(delim: "(", 1, dots.h.c, N; s (1), dots.h.c, s (N); #none) = mat(delim: "(", s (1), dots.h.c, s (N)) $
...
$i eq.not j$を満たす$i , j = 1 , dots.h.c , N$に対して、$i$と$j$を入れ替えてそれ以外はそのままにする置換を$i$と$j$の#emph[互換];といい$mat(delim: "(", i, j)$と表す。
つまり、
$ mat(delim: "(", i, j) = mat(delim: "(", dots.h.c, i, dots.h.c, j dots.h.c; dots.h.c, j, dots.h.c, i dots.h.c; #none) $
である。
...
$ S_2 = { mat(delim: "(", 1, 2) , mat(delim: "(", 2, 1) } , $
$ S_2 (0) = { mat(delim: "(", 1, 2) } , quad S_2 (1) = { mat(delim: "(", 2, 1) } . $
$N = 3$の時は
$ S_3 = { mat(delim: "(", 1, 2, 3) , mat(delim: "(", 2, 1, 3) , mat(delim: "(", 1, 3, 2) , mat(delim: "(", 3, 2, 1) , mat(delim: "(", 2, 3, 1) , mat(delim: "(", 3, 1, 2) } , $
$ S_3 (0) = { mat(delim: "(", 1, 2, 3) } , quad S_3 (1) = { mat(delim: "(", 2, 1, 3) , mat(delim: "(", 1, 3, 2) , mat(delim: "(", 3, 2, 1) } , quad S_3 (2) = { mat(delim: "(", 2, 3, 1) , mat(delim: "(", 3, 1, 2) } . $
...
#block[
#emph[Proof.] $t = mat(delim: "(", 1, 2)$のとき、 \$\$\\sgn(t)
...
= -1.\$\$
一般の互換$t = mat(delim: "(", k, l)$に対しては、$mat(delim: "(", l, k) = mat(delim: "(", k, l)$より$k < l$としてよく、置換$s$を
\$\$s =
\\begin{cases}
\\id & (k = 1, l = 2), \\\\
...
\\end{cases}\$\$
とすると$t = s^(- 1) mat(delim: "(", 1, 2) s$であるので、
...
#block[
$N$次の横ベクトル\$\\vb\*{a}\_1, \\cdots, \\vb\*{a}\_N, \\vb\*{a}\'\_1, \\cdots, \\vb\*{a}\'\_N\$とスカラー$c_1 , dots.h.c , c_N , c'_1 , dots.h.c , c'_N$に対して、
\$\$\\begin{vmatrix}\\vdots \\\\ c\_i\\vb\*{a}\_i+c\'\_i\\vb\*{a}\'\_i \\\\ \\vdots\\end{vmatrix}
= c\_i\\begin{vmatrix}\\vdots \\\\ \\vb\*{a}\_i \\\\ \\vdots\\end{vmatrix}+c\'\_i\\begin{vmatrix}\\vdots \\\\ \\vb\*{a}\'\_i \\\\ \\vdots\\end{vmatrix}\$\$
が成り立つ。
また、$N$次の縦ベクトル\$\\vb\*{a}\_1, \\cdots, \\vb\*{a}\_N, \\vb\*{a}\'\_1, \\cdots, \\vb\*{a}\'\_N\$とスカラー$c_1 , dots.h.c , c_N , c'_1 , dots.h.c , c'_N$に対して、
\$\$\\begin{vmatrix}\\cdots & c\_i\\vb\*{a}\_i+c\'\_i\\vb\*{a}\'\_i & \\cdots\\end{vmatrix}
= c\_i\\begin{vmatrix}\\cdots & \\vb\*{a}\_i & \\cdots\\end{vmatrix}+c\'\_i\\begin{vmatrix}\\cdots & \\vb\*{a}\'\_i & \\cdots\\end{vmatrix}\$\$
が成り立つ。
...
#block[
$N$次の横ベクトル\$\\vb\*{a}\_1, \\cdots, \\vb\*{a}\_N\$と$i eq.not j$を満たす$i , j = 1 , dots.h.c , N$に対して
...
が成り立つ。
また、$N$次の縦ベクトル\$\\vb\*{a}\_1, \\cdots, \\vb\*{a}\_N, \\vb\*{a}\$と$i eq.not j$を満たす$i , j = 1 , dots.h.c , N$に対して
...
ここで奇置換の方にだけ互換$t = mat(delim: "(", i, j)$の操作をすると、$A$の仮定から
...
=
\\begin{pmatrix}
a\_{1 1}\\vb\*{b}\_1+\\cdots+a\_{1 N}\\vb\*{b}\_N \\\\
\\vdots   \\\\
a\_{N 1}\\vb\*{b}\_1+\\cdots+a\_{N N}\\vb\*{b}\_N \\\\
\\end{pmatrix}.\$\$ よって、 \$\$\\det(A B) =
\\begin{vmatrix}
\\sum\_{j\_1 = 1}^N a\_{1 j\_i}\\vb\*{b}\_{j\_1} \\\\
\\vdots   \\\\
\\sum\_{j\_N = 1}^N a\_{N j\_N}\\vb\*{b}\_{j\_N} \\\\
\\end{vmatrix}.\$\$ 多重線形性より、 \$\$\\det(A B) =
\\sum\_{j\_1 = 1}^N \\cdots \\sum\_{j\_N = 1}^N a\_{1 j\_1}\\cdots a\_{N j\_N}\\begin{vmatrix}\\vb\*{b}\_{j\_1} \\\\ \\vdots \\\\ \\vb\*{b}\_{j\_N}\\end{vmatrix}.\$\$
交代性より同じ行がある場合の行列式は$0$なので、 \$\$\\det(A B) =
\\sum\_{s \\in S\_N} a\_{1 s(1)}\\cdots a\_{N s(N)}\\begin{vmatrix}\\vb\*{b}\_{s(1)} \\\\ \\vdots \\\\ \\vb\*{b}\_{s(N)}\\end{vmatrix}.\$\$
行を並べ替えて、 \$\$\\det(A B) =
\\sum\_{s \\in S\_N} \\sgn(s)a\_{1 s(1)}\\cdots a\_{N s(N)}\\begin{vmatrix}\\vb\*{b}\_1 \\\\ \\vdots \\\\ \\vb\*{b}\_N\\end{vmatrix}.\$\$
...
つまり、$A$を
$ A = mat(delim: "(", A_(U L), \*, A_(U R); \*, a_(i j), \*; A_(L L), \*, A_(L R); #none) $
と区分けした時に
...
$ mat(delim: "(", a_(i 1), a_(i 2), dots.h.c, a_(i N)) = a_(i 1) mat(delim: "(", 1, 0, dots.h.c, 0) + a_(i 2) mat(delim: "(", 0, 1, dots.h.c, 0) + a_(i N) mat(delim: "(", 0, 0, dots.h.c, 1) $
と分解すると、多重線形性より示される。
...
$ tilde(A) = mat(delim: "(", tilde(A)_(1 1), dots.h.c, tilde(A)_(N 1); dots.v, dots.down, dots.v; tilde(A)_(1 N), dots.h.c, tilde(A)_(N N); #none) $
...
#block[
<t:cofactvec>
$A$を$N$次正方行列、$tilde(A)$をその余因子行列として\$\\vb\*{b}\$を$N$次ベクトルとする。
この時、 \$\$\\tilde{A}\\vb\*{b}
= \\begin{pmatrix}\\det A\_{1, \\vb\*{b}} \\\\ \\vdots \\\\ \\det A\_{N, \\vb\*{b}}\\end{pmatrix}\$\$
が成り立つ。
ただし、\$A\_{k, \\vb\*{b}}\$は行列\$A = \\begin{pmatrix}\\vb\*{a}\_1 & \\cdots & \\vb\*{a}\_N\\end{pmatrix}\$の第$k = 1 , dots.h.c , N$列をベクトル\$\\vb\*{b}\$で置き換えて得られる行列
\$\$A\_{k, \\vb\*{b}} = \\begin{pmatrix}\\vb\*{a}\_1 & \\cdots & \\vb\*{a}\_{k-1} & \\vb\*{b} & \\vb\*{a}\_{k+1} & \\cdots & \\vb\*{a}\_N\\end{pmatrix}\$\$
である。

]
#block[
#emph[Proof.] ベクトル\$\\tilde{A}\\vb\*{b}\$の第$k$成分は
$ tilde(A)_(1 j) b_1 + dots.h.c + tilde(A)_(N j) b_N $
であり、これは\$\\det A\_{k, \\vb\*{b}}\$の第$k$列についての余因子展開に一致する。~◻

]
#block[
#emph[定理@t:cofactinvの証明.]
$tilde(A) A = det (A) I_N$を示せば十分である。
$tilde(A) A$の第$(i , j)$成分は、$A$の第$j$列を\$\\vb\*{a}\_j\$とすると、\$\\tilde{A}\\vb\*{a}\_j\$の第$i$成分なので、補題@t:cofactvecより、\$\\det A\_{i, \\vb\*{a}\_j}\$に等しい。
...
\$\$A\\vb\*{x} = \\vb\*{b}\$\$
の一意な解\$\\vb\*{x} = \\begin{pmatrix}x\_1 \\\\ \\vdots \\\\ x\_N\\end{pmatrix}\$の成分$x_k$は
\$\$x\_k = \\frac{\\det A\_{k, \\vb\*{b}}}{\\det A}\$\$ で与えられる。

]
#block[
#emph[Proof.]
$A$は逆行列を持ち、それは余因子行列を用いて$A = frac(1, det A) tilde(A)$で与えられるので、解は\$\\vb\*{x} = \\frac{1}{\\det A}\\tilde{A}\\vb\*{b}\$である。
